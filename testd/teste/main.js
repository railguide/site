    ...
    var HOVER_POPUP_CONFIG = {
        0: { hoverFields: ["rr_full", "rg_name"], popupFields: ["rr_full", "rg_name", "sub_title", "rg_desc"], popupFieldStyles: { rg_name: "popup-field-title", sub_title: "popup-field-sub" }, hoverFieldStyles: { rg_name: "hover-field-title" } },
        ...
        2: { hoverFields: ["classic_full", "rg_name", "sub_title", "rg_desc"], popupFields: ["classic_full", "rg_name", "sub_title", "rg_desc"], popupFieldStyles: { rg_name: "popup-field-title", sub_title: "popup-field-sub" }, hoverFieldStyles: { rg_name: "hover-field-title", sub_title: "hover-field-sub" } },
    ...
    function buildHoverHtml ( properties, options ) {
        ...
        for (var i = 0; i < fieldKeys.length; i++) {
            ...
            if (fieldStyles[key]) cssClass += " " + fieldStyles[key];
            parts.push("<span class=\"" + cssClass + "\">" + inner + "</span>");
        }
    ...
    class handleMapPrcoessCreation {
        ...
        addEvents () {
            ...
            this.map.on("style.load", runBottomLayout);
            ...
            if (hoverTooltip) {
                this.map.on("mousemove", function (e) {
                    ...
                    for (var i = 0; i < features.length; i++) {
                        ...
                        if (!p) continue;
                        if (useHoverStyles) {
                            labelHtml = buildHoverHtml(p, { hoverFields: cfg.hoverFields, hoverMaxChars: cfg.hoverMaxChars, hoverMaxLines: cfg.hoverMaxLines, hoverFieldStyles: cfg.hoverFieldStyles });
                            if (labelHtml && labelHtml.replace(/<[^>]+>/g, "").trim()) break;
                        } else {
                    ...
                    if ((useHoverStyles && labelHtml) || (!useHoverStyles && label && String(label).trim())) {
                        base.map.getCanvas().style.cursor = "pointer";
                        if (useHoverStyles) {
                            hoverTooltip.innerHTML = labelHtml;
                        } else {
            ...
            this.map.on("click", function (e) {
                ...
                for (var i = 0; i < features.length; i++) {
                    var p = features[i].properties;
                    if (!p) continue;
                    label = p.rr_full || p.classic_full || p.rg_name || p.sub_title || p.rg_desc || p.name || p.title || p.label || p.Name || p.Title || p.description || "";
